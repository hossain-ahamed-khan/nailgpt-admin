...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:relative w-64 h-screen lg:h-auto bg-gray-50 border-r border-gray-200 p-4 sm:p-6 flex flex-col z-40 transition-transform duration-300 ${
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className={`rounded-lg p-3 sm:p-4 flex items-center gap-3 cursor-pointer transition ${
                active
                  ? 'bg-yellow-400 hover:bg-yellow-500'
                  : 'text-gray-600 hover:bg-gray-100'
              }`}>
                <Icon className={`w-5 h-5 shrink-0 ${active ? 'text-gray-900' : ''}`} />
                <span className={`text-xs sm:text-sm font-${active ? 'semibold' : 'medium'} ${active ? 'text-gray-900' : ''}`}>
                  {item.label}
                </span>
              </div>
            </Link>
